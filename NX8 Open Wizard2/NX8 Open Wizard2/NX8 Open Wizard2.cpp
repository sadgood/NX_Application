...
extern DllExport void ufusr(char *parm, int *returnCode, int rlen)
{
	...
	/* TODO: Add your application code here */
	{
		...
		do
		{
			...
			if (resp == UF_UI_CB_TERMINATE)
			{
				UF_ugUIbeginSpecial();
				UF_UI_launch_meshpoint_dialog();
				UF_ugUIendSpecial();
#if 1
				char *rootDir = NULL;
				...
				char dllPath[1024] = "";
				sprintf_s(dllPath, 1024, "%s\\libugui.dll", rootDir);

				typedef void (WINAPI *MyFun)(char const *);

				MyFun fun = NULL;
				HINSTANCE hHandle = NULL;
				hHandle = LoadLibraryA(dllPath);
				fun = (MyFun)::GetProcAddress(hHandle, "?MACRO_playback_from_usertool@@YAXPBD@Z");

				if (fun != NULL)
				{
					fun("f:\\macro\\hard_point.macro");
				}

#endif
			}
		} while (resp == UF_UI_CB_TERMINATE);
	}
